<script lang="ts">
	...
	let dedupeResult = $state<{ duplicatesRemoved: number; remainingDuplicates: number } | null>(null)
...
<div class="bg-zinc-50 border border-zinc-200 rounded-xl px-4 py-3 space-y-3 text-xs">
	...
	<div class="flex items-center gap-2 flex-wrap">
		...
		<select
			bind:value={deleteType}
			class="border border-zinc-300 rounded px-2 py-0.5 bg-white"
		>
	...
	<div class="flex items-center gap-2 flex-wrap">
		...
		>
			{#if statusDedupe.state === 'loading'}
				<span class="w-3 h-3 border-2 border-amber-300 border-t-transparent rounded-full animate-spin"></span>
			{/if}
			Deduplicate graph
		</button>
	...
	<!-- Status data -->
	{#if statusInfoData}
		<div class="rounded-lg border border-zinc-200 bg-white px-3 py-2 font-mono text-[11px] text-zinc-600 overflow-x-auto">
	...
	{#if examplesList.length > 0}
		<div class="flex flex-wrap gap-1.5">
			{#each examplesList as q (q)}
				<span class="px-2 py-1 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-600">{q}</span>
			{/each}
		</div>
	{/if}
</div>
