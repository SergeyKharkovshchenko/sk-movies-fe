<script lang="ts">
	...
	// Guards the two onscroll handlers below from bouncing off each other -- syncScroll sets a
	// container's scrollTop programmatically, which itself fires that container's own scroll
	// event; without this flag that would immediately re-trigger a sync back the other way.
	...
	function handleMessagesScroll() {
		if (panelOpen && panelEl) syncScroll(messagesEl, panelEl);
	}

	function handlePanelScroll() {
		if (panelEl) syncScroll(panelEl, messagesEl);
	}
	...
	// assistant message. Shared by normal sends and by 'compare' mode, which calls this twice and
	// uses the returned content to feed the judge call afterward.
	...
	// Judges the two answers 'compare' mode just generated and appends a result card to the
	// side panel. Runs after both sendOne() calls resolve, not woven into either of them, since
	// it needs both finished answers as input.
	async function analyzeComparison(
		...
		try {
			const res = await KnowledgeAPIService.compareAnalyze({
				question,
				answerA,
				labelA: modeA,
				answerB,
			...
			comparisons = comparisons.map((c) =>
				c.id === id ? { ...c, loading: false, error: true } : c
			);
		}
	}
...
<div class="flex h-full gap-0">
	<div
		class="flex flex-1 min-w-0 flex-col h-full bg-white rounded-xl border border-zinc-200 shadow-sm overflow-hidden"
	>
		<!-- Header -->
					...
					<div class="space-y-1 max-h-56 overflow-y-auto pr-1">
						...
						{#each probeQuestions as item, i (i)}
							...
							>
								...
								<p
									class="text-zinc-400 group-hover:text-indigo-500 text-[10px] leading-snug mt-0.5"
								>
									{item.why}
								</p>
							</button>
	...
	{#if comparisons.length > 0}
		<button
			...
			class="shrink-0 w-4 flex items-center justify-center border-y border-zinc-200 bg-zinc-50 hover:bg-zinc-100 transition-colors rounded-sm"
			aria-label={panelOpen ? 'Collapse comparison panel' : 'Expand comparison panel'}
			title={panelOpen ? 'Collapse comparison panel' : 'Expand comparison panel'}
		>
			<svg
		...
		<div
			...
			class="overflow-y-auto rounded-xl bg-zinc-50 transition-[width] duration-200 ease-in-out shrink-0"
			class:w-96={panelOpen}
			class:w-0={!panelOpen}
			class:border={panelOpen}
			class:border-zinc-200={panelOpen}
			class:px-3={panelOpen}
			class:py-4={panelOpen}
		>
			{#if panelOpen}
				<h3 class="text-xs font-semibold text-zinc-500 uppercase tracking-wide px-1 mb-3">
					Compare Analysis
				</h3>
				<div class="space-y-3">
					{#each comparisons as c (c.id)}
						<div class="rounded-xl border border-zinc-200 bg-white p-3 text-xs space-y-2">
							<p class="text-zinc-500 font-medium line-clamp-2" title={c.question}>{c.question}</p>
							{#if c.loading}
							...
							{:else if c.error}
								<p class="text-red-500">Failed to analyze this comparison.</p>
							{:else}
								<div class="flex items-center gap-1.5">
									<span class="text-zinc-400">Verdict:</span>
									<span
										class="px-1.5 py-0.5 rounded font-medium"
										class:bg-indigo-100={c.verdict === c.modeA}
										class:text-indigo-600={c.verdict === c.modeA}
										class:bg-zinc-100={c.verdict === c.modeB}
										class:text-zinc-600={c.verdict === c.modeB}
										class:bg-amber-100={c.verdict === 'tie'}
										class:text-amber-700={c.verdict === 'tie'}
									>
										{c.verdict}
									</span>
								</div>
