<script lang="ts">
	...
	let cacheStates = $state<Record<CacheKey, OpState>>({ all: 'idle', suggestGraph: 'idle', suggestSections: 'idle' })
	...
	async function clearCache(key: CacheKey) {
		...
		try {
			...
			setTimeout(() => { cacheStates = { ...cacheStates, [key]: 'idle' } }, 2000)
		} catch {
...
<div class="rounded-xl border border-zinc-200 bg-white text-xs overflow-hidden">
	...
	>
		<span class="font-semibold text-zinc-500 uppercase tracking-wide text-[11px]">Admin — Status & Labels</span>
		<svg
	...
	{#if open}
		<div class="px-4 py-3 grid grid-cols-2 gap-6">
				...
				<div class="flex items-center justify-between">
					...
					>
						{#if statusState === 'loading'}
							<span class="w-2.5 h-2.5 border-2 border-zinc-400 border-t-transparent rounded-full animate-spin"></span>
						{/if}
						Refresh
					</button>
				</div>
						...
						<table class="w-full text-[11px]">
							<thead>
								<tr class="bg-zinc-50 text-zinc-500">
									<th class="text-left px-2 py-1.5 font-medium" title="Knowledge base name — groups Neo4j graph, Postgres text chunks, and vector embeddings for one topic">Knowledge Base</th>
									<th class="text-right px-2 py-1.5 font-medium" title="Neo4j graph nodes">Nodes</th>
									<th class="text-right px-2 py-1.5 font-medium" title="Neo4j relationships">Rels</th>
									<th class="text-right px-2 py-1.5 font-medium" title="Vector embeddings (pgvector)">Emb</th>
								</tr>
							</thead>
							<tbody>
			...
			<div class="space-y-2">
				<div class="flex items-center justify-between">
					<span class="font-medium text-zinc-600" title="Each knowledge base is a named dataset: Neo4j graph + Postgres text chunks + pgvector embeddings">Knowledge Bases</span>
					<button
						onclick={fetchLabels}
				...
				{#if labelsList.length > 0}
					<div class="flex flex-wrap gap-1.5">
						{#each labelsList as label (label)}
							<div class="flex items-center gap-1 pl-2.5 pr-1 py-0.5 rounded-full border border-zinc-200 bg-zinc-50 font-mono text-zinc-700">
								{label}
								<button
		...
		<div class="px-4 pb-3 border-t border-zinc-100 pt-3">
			<div class="flex items-center gap-3 flex-wrap">
				...
				{#each ([
					{ key: 'suggestGraph', label: 'suggestGraph', title: 'Clear cached suggest-graph LLM responses' },
					{ key: 'suggestSections', label: 'suggestSections', title: 'Clear cached suggest-sections LLM responses' },
					{ key: 'all', label: 'Clear all', title: 'Clear all LLM response caches (suggestGraph + suggestSections)' },
